...
@utils.elapsed_time
def get_file_list(
        s3_bucket: str, product: str, dataset_id: str, dataset_version: str, file_filter: str, subdir: str,
        recursive: bool
):
    my_s3 = s3_singleton.S3()
    return my_s3.file_list(s3_bucket, product, dataset_id, dataset_version, file_filter, subdir, recursive)


@utils.elapsed_time
def download_files(
        s3_bucket: str, product: str, dataset_id: str, dataset_version: str, file_filter: str, output_directory: str,
        overwrite: bool = False, keep_timestamps: bool = False, subdir: str = None,
        sync_etag: bool = False, sync_time: bool = False, n_threads=None, recursive: bool = False
):
    files_list = get_file_list(s3_bucket, product, dataset_id, dataset_version, file_filter, subdir, recursive)

    if len(files_list) == 0:
        logger.error(f'No match found for {file_filter} in {product}/{dataset_id}_{dataset_version}')
        raise FileNotFoundError(f'No match found for {file_filter} in {product}/{dataset_id}_{dataset_version}')

    if not n_threads:
        _download(files_list, keep_timestamps, output_directory, overwrite, sync_etag, sync_time)
    else:
        ...
        if n_threads != len(files_list_split):
            raise ValueError(f'Mismatch between number of threads ({n_threads}) '
                             f'and threads parameters ({len(files_list_split)})')

        # build input args for each threads
        threads_args = [
            [
                files_list_split[i], keep_timestamps, output_directory, overwrite, sync_etag, sync_time
            ] for i in range(n_threads)
        ]

        with Pool(processes=10) as pool:
            # start threads
            results: List = [pool.apply_async(_download, thread_args) for thread_args in threads_args]
            try:
                for r in results:
...
def _download(files_list, keep_timestamps, output_directory, overwrite, sync_etag, sync_time):
    ...
    try:
        ...
        for s3_file in files_list:
            ...
            if not file_can_be_downloaded(s3_file, dest_file, overwrite, sync_time, sync_etag):
            ...
            if keep_timestamps:
                logger.info(f'Set original last modified: {s3_file.last_modified} to {dest_file}')
...
def file_can_be_downloaded(s3_file: S3File, dest_file: str, overwrite: bool, sync_time: bool, sync_etag: bool) -> bool:
